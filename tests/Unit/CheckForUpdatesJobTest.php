...
it('uses max of CDN and cache versions', function () {
    ...
    File::shouldReceive('get')
        ->with(base_path('versions.json'))
        ->andReturn(json_encode(['saturn' => ['v4' => ['version' => '4.0.10']]]));
    ...
    File::shouldReceive('put')
        ...
        ->with(base_path('versions.json'), Mockery::on(function ($json) {
            ...
            return $data['saturn']['v4']['version'] === '4.0.10';
        }));
...
it('never downgrades from current running version', function () {
    ...
    File::shouldReceive('get')
        ->with(base_path('versions.json'))
        ->andReturn(json_encode(['saturn' => ['v4' => ['version' => '4.0.5']]]));
...
it('preserves other component versions when preventing Saturn Platform downgrade', function () {
    // CDN has older Saturn Platform but newer Traefik
    Http::fake([
        '*' => Http::response([
            'saturn' => ['v4' => ['version' => '4.0.0']],
            'traefik' => ['v3.6' => '3.6.2'],
    ...
    File::shouldReceive('put')
        ...
        ->with(base_path('versions.json'), Mockery::on(function ($json) {
            $data = json_decode($json, true);
            // Saturn Platform should use running version
            expect($data['saturn']['v4']['version'])->toBe('4.0.10');
    ...
    \Illuminate\Support\Facades\Log::shouldReceive('warning')
        ->once()
        ->with('CDN served older Saturn Platform version than cache', Mockery::type('array'));
