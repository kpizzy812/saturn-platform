 ...
 */
it('verifies docker_compose_raw does not contain Saturn Platform labels after parsing', function () {
...
YAML;
    ...
    // Check that docker_compose_raw does NOT contain Saturn Platform labels
    $labels = data_get($yamlAfterParsing, 'services.web.labels', []);
    $hasTraefikLabels = false;
    $hasSaturnManagedLabel = false;
    ...
    foreach ($labels as $label) {
        if (is_string($label)) {
            ...
            if (str_contains($label, 'saturn.managed')) {
                $hasSaturnManagedLabel = true;
            }
        }
    }

    // docker_compose_raw should NOT have Saturn Platform additions
    expect($hasTraefikLabels)->toBeFalse('docker_compose_raw should not contain Traefik labels');
    expect($hasSaturnManagedLabel)->toBeFalse('docker_compose_raw should not contain saturn.managed label');
