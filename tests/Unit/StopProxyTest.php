...
it('ensures stop proxy includes wait loop for container removal', function () {
    ...
    $commands = [
        'docker stop -t 30 saturn-proxy 2>/dev/null || true',
    ...
    expect($commandsString)->toContain('docker stop -t 30 saturn-proxy')
        ->and($commandsString)->toContain('docker rm -f saturn-proxy')
        ->and($commandsString)->toContain('for i in {1..10}; do')
        ->and($commandsString)->toContain('if ! docker ps -a --format "{{.Names}}" | grep -q "^saturn-proxy$"')
...
it('uses configurable timeout for docker stop', function () {
    ...
    $timeout = 30;
    $stopCommand = "docker stop -t $timeout saturn-proxy 2>/dev/null || true";
...
it('waits for swarm service container removal correctly', function () {
    ...
    $containerName = 'saturn-proxy_traefik';
    $checkCommand = "    if ! docker ps -a --format \"{{.Names}}\" | grep -q \"^$containerName$\"; then";

    expect($checkCommand)->toContain('saturn-proxy_traefik');
});
