...
// type() Tests
test('type returns standalone-clickhouse', function () {
    $db = new StandaloneClickhouse;
    expect($db->type())->toBe('standalone-clickhouse');
});

// databaseType() Tests
test('databaseType accessor returns type value', function () {
    $db = new StandaloneClickhouse;
    expect($db->database_type)->toBe('standalone-clickhouse');
});

// Fillable Security Tests
test('fillable does not contain id', function () {
    $fillable = (new StandaloneClickhouse)->getFillable();
...
test('fillable contains expected ClickHouse fields', function () {
    $fillable = (new StandaloneClickhouse)->getFillable();

    expect($fillable)
        ->toContain('name')
        ->toContain('clickhouse_user')
        ->toContain('clickhouse_password')
        ->toContain('clickhouse_db')
        ->toContain('image')
        ->toContain('status')
        ->toContain('is_public')
...
// Casts Tests
test('clickhouse_password is cast to encrypted', function () {
    $casts = (new StandaloneClickhouse)->getCasts();

    expect($casts['clickhouse_password'])->toBe('encrypted');
});
...
test('portsMappings setter preserves non-empty string', function () {
    ...
    expect($db->getAttributes()['ports_mappings'])->toBe('9000:9000');
});
...
test('portsMappingsArray returns array when ports_mappings is comma string', function () {
    $db = new StandaloneClickhouse;
    $db->setRawAttributes(['ports_mappings' => '9000:9000,8123:8123'], true);
