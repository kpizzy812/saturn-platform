...
// type() Tests
test('type returns standalone-dragonfly', function () {
    $db = new StandaloneDragonfly;
    expect($db->type())->toBe('standalone-dragonfly');
});

// databaseType() Tests
test('databaseType accessor returns type value', function () {
    $db = new StandaloneDragonfly;
    expect($db->database_type)->toBe('standalone-dragonfly');
});

// Fillable Security Tests
test('fillable does not contain id', function () {
    $fillable = (new StandaloneDragonfly)->getFillable();
...
test('fillable contains expected Dragonfly fields', function () {
    $fillable = (new StandaloneDragonfly)->getFillable();

    expect($fillable)
        ->toContain('name')
        ->toContain('dragonfly_password')
        ->toContain('image')
        ->toContain('status')
        ->toContain('is_public')
...
// Casts Tests
test('dragonfly_password is cast to encrypted', function () {
    $casts = (new StandaloneDragonfly)->getCasts();

    expect($casts['dragonfly_password'])->toBe('encrypted');
});
