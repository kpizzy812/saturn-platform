 ...
 * 2. SATURN_* environment variables with null or empty values are filtered out
...
it('filters out null environment variables from nixpacks build command', function () {
    ...
    $job->shouldReceive('generate_saturn_env_variables')
        ->andReturn(collect([
            'SATURN_FQDN' => 'example.com',
            'SATURN_URL' => null,  // null value that should be filtered
            'SATURN_BRANCH' => '',  // empty value that should be filtered
    ...
    expect($envArgs)->toContain('--env SATURN_FQDN=example.com');
    ...
    expect($envArgs)->not->toContain('SATURN_URL');
    expect($envArgs)->not->toContain('SATURN_BRANCH');
...
it('filters out null environment variables from nixpacks preview deployments', function () {
    ...
    $job->shouldReceive('generate_saturn_env_variables')
        ->andReturn(collect([
            'SATURN_FQDN' => 'preview.example.com',
        ]));
    ...
    expect($envArgs)->toContain('--env SATURN_FQDN=preview.example.com');
