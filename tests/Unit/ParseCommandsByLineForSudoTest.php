...
test('adds ownership changes for Saturn Platform data paths', function () {
    $commands = collect([
        'mkdir -p /data/saturn/logs',
    ]);
    ...
    expect($result[0])->toBe('sudo mkdir -p /data/saturn/logs && sudo sudo chown -R ubuntu:ubuntu /data/saturn/logs && sudo sudo chmod -R o-rwx /data/saturn/logs');
});

test('adds ownership changes for Saturn Platform tmp paths', function () {
    $commands = collect([
        'mkdir -p /tmp/saturn/cache',
    ]);
    ...
    // Note: The && operator adds another sudo, creating double sudo for chown/chmod
    // This is existing behavior that may need refactoring but isn't part of this bug fix
    expect($result[0])->toBe('sudo mkdir -p /tmp/saturn/cache && sudo sudo chown -R ubuntu:ubuntu /tmp/saturn/cache && sudo sudo chmod -R o-rwx /tmp/saturn/cache');
});
