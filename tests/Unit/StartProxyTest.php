...
it('ensures container cleanup includes wait loop in command sequence', function () {
    ...
    expect($commandsString)->toContain('docker stop saturn-proxy 2>/dev/null || true')
        ->and($commandsString)->toContain('docker rm -f saturn-proxy 2>/dev/null || true')
        ->and($commandsString)->toContain('for i in {1..10}; do')
        ->and($commandsString)->toContain('if ! docker ps -a --format "{{.Names}}" | grep -q "^saturn-proxy$"; then')
    ...
    $stopPosition = strpos($commandsString, 'docker stop saturn-proxy');
