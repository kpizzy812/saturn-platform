...
**Файл:** `app/Services/RepositoryAnalyzer/InfrastructureProvisioner.php`

## Обязанности

1. Создание баз данных через существующие хелперы
2. Создание приложений с правильной конфигурацией
3. Связывание через ResourceLink с `auto_inject = true`
4. Генерация env переменных из `.env.example`
...
```
1. Databases (PostgreSQL, Redis, etc.)
   ↓
2. Applications (with base_directory from monorepo path)
   ↓
3. ResourceLinks (connect apps to databases)
   ↓
4. Environment Variables (from .env.example)
   ↓
5. Queue Deployments (ApplicationDeploymentJob)
   ↓
6. Start Databases (StartDatabase::dispatch)
```

---

## Связь с существующими хелперами

| Тип БД | Хелпер функция | Файл |
|--------|---------------|------|
| PostgreSQL | `create_standalone_postgresql()` | `bootstrap/helpers/databases.php` |
| MySQL | `create_standalone_mysql()` | `bootstrap/helpers/databases.php` |
| MongoDB | `create_standalone_mongodb()` | `bootstrap/helpers/databases.php` |
| Redis | `create_standalone_redis()` | `bootstrap/helpers/databases.php` |
| ClickHouse | `create_standalone_clickhouse()` | `bootstrap/helpers/databases.php` |
