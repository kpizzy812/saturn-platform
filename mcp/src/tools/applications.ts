    ...
    server.registerTool(
        'saturn_create_application',
        {
            ...
            inputSchema: z.object({
                ...
                environment_uuid: z.string().optional().describe('Environment UUID (required for private-github-app and private-deploy-key types)'),
                ...
                github_app_uuid: z.string().optional().describe('GitHub App UUID — required for private-github-app type. Get from saturn_list_github_apps.'),
                private_key_uuid: z.string().optional().describe('Deploy key UUID — required for private-deploy-key type. Get from saturn_list_ssh_keys.'),
    ...
    server.registerTool(
        'saturn_update_application',
        {
            ...
            inputSchema: z.object({
                ...
                base_directory: z.string().optional().describe('Base directory for all commands (e.g. "/" or "apps/api")'),
                dockerfile_location: z.string().optional().describe('Path to Dockerfile (e.g. "apps/api/Dockerfile")'),
                docker_compose_location: z.string().optional().describe('Path to docker-compose file (e.g. "docker-compose.yml")'),
                docker_compose_raw: z.string().optional().describe('Raw Docker Compose content'),
                docker_compose_custom_start_command: z.string().optional().describe('Docker Compose custom start command'),
                docker_compose_custom_build_command: z.string().optional().describe('Docker Compose custom build command'),
                docker_compose_domains: z.record(z.object({ domain: z.string() })).optional().describe('Per-service FQDN mapping for docker compose deployments, e.g. {"frontend": {"domain": "app.example.com"}, "api": {"domain": "api.example.com"}}'),
