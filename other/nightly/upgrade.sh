...
STATUS_FILE="/data/saturn/source/.upgrade-status"

DATE=$(date +%Y-%m-%d-%H-%M-%S)
LOGFILE="/data/saturn/source/upgrade-${DATE}.log"
...
echo ""
echo "=========================================="
echo "   Saturn Platform Upgrade - ${DATE}"
echo "=========================================="
echo ""

# Initialize log file with header
echo "============================================================" >>"$LOGFILE"
echo "Saturn Platform Upgrade Log" >>"$LOGFILE"
...
COMPOSE_FILES="-f /data/saturn/source/docker-compose.yml -f /data/saturn/source/docker-compose.prod.yml"

# Check if custom compose file exists
if [ -f /data/saturn/source/docker-compose.custom.yml ]; then
    COMPOSE_FILES="$COMPOSE_FILES -f /data/saturn/source/docker-compose.custom.yml"
...
# Make sure saturn network exists
# It is created when starting Saturn Platform with docker compose
log "Checking Docker network 'saturn'..."
if ! docker network inspect saturn >/dev/null 2>&1; then
    log "Network 'saturn' does not exist, creating..."
    if ! docker network create --attachable --ipv6 saturn 2>/dev/null; then
        log "Failed to create network with IPv6, trying without IPv6..."
        docker network create --attachable saturn 2>/dev/null
        log "Network 'saturn' created without IPv6"
    else
        log "Network 'saturn' created with IPv6 support"
    fi
else
    log "Network 'saturn' already exists"
fi
...
HELPER_IMAGE="${REGISTRY_URL:-ghcr.io}/saturnplatform/saturn-helper:${LATEST_HELPER_VERSION}"
...
echo "     This step will restart all Saturn Platform containers."
echo "     Check the log file for details: ${LOGFILE}"

# From this point forward, we need to ensure the script continues even if
# the SSH connection is lost (which happens when saturn container stops)
...
nohup bash -c "
    ...
    for container in saturn saturn-db saturn-redis saturn-realtime; do
    ...
    if [ -f /data/saturn/source/docker-compose.custom.yml ]; then
        ...
        docker run -v /data/saturn/source:/data/saturn/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${REGISTRY_URL:-ghcr.io}/saturnplatform/saturn-helper:\${LATEST_HELPER_VERSION} bash -c \"LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/saturn/source/.env -f /data/saturn/source/docker-compose.yml -f /data/saturn/source/docker-compose.prod.yml -f /data/saturn/source/docker-compose.custom.yml up -d --remove-orphans --wait --wait-timeout 60\" >>\"\$LOGFILE\" 2>&1
    else
        log 'Using standard docker-compose configuration'
        log 'Running docker compose up...'
        docker run -v /data/saturn/source:/data/saturn/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${REGISTRY_URL:-ghcr.io}/saturnplatform/saturn-helper:\${LATEST_HELPER_VERSION} bash -c \"LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/saturn/source/.env -f /data/saturn/source/docker-compose.yml -f /data/saturn/source/docker-compose.prod.yml up -d --remove-orphans --wait --wait-timeout 60\" >>\"\$LOGFILE\" 2>&1
    fi
    ...
    log 'Saturn Platform upgrade completed successfully'
...
echo ""
echo "=========================================="
echo "   Saturn Platform upgrade to ${LATEST_IMAGE} in progress"
echo "=========================================="
echo ""
echo "   The upgrade will continue in the background."
echo "   Saturn Platform will be available again shortly."
