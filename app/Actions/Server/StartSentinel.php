...
class StartSentinel
{
    ...
    public function handle(Server $server, bool $restart = false, ?string $latestVersion = null, ?string $customImage = null)
    {
        ...
        $mountDir = '/data/saturn/sentinel';
        $image = config('constants.saturn.registry_url').'/coollabsio/sentinel:'.$version;
        if (! $endpoint) {
        ...
        $dockerCommand = "docker run -d $dockerEnvironments --name saturn-sentinel -v /var/run/docker.sock:/var/run/docker.sock -v $mountDir:/app/db --pid host --health-cmd \"curl --fail http://127.0.0.1:8888/api/health || exit 1\" --health-interval 10s --health-retries 3 --add-host=host.docker.internal:host-gateway --label $dockerLabels $image";

        instant_remote_process([
            'docker rm -f saturn-sentinel || true',
            "mkdir -p $mountDir",
            $dockerCommand,
