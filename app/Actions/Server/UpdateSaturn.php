...
class UpdateSaturn
{
    ...
    public function handle($manual_update = false)
    {
        ...
        try {
            $response = Http::retry(3, 1000)->timeout(10)
                ->get(config('constants.saturn.versions_url'));
            ...
            if ($response->successful()) {
                ...
                if ($cacheVersion && version_compare($cacheVersion, config('constants.saturn.version'), '<')) {
                    ...
                    throw new \Exception(
                        ...
                        "({$cacheVersion}) is older than running version (".config('constants.saturn.version').')'
                    );
                }
        ...
        $this->currentVersion = config('constants.saturn.version');
        if (! $manual_update) {
    ...
    private function update()
    {
        ...
        $upgradeScriptUrl = config('constants.saturn.upgrade_script_url');

        remote_process([
            "curl -fsSL {$upgradeScriptUrl} -o /data/saturn/source/upgrade.sh",
            "bash /data/saturn/source/upgrade.sh $this->latestVersion $latestHelperImageVersion",
        ], $this->server);
    }
}
