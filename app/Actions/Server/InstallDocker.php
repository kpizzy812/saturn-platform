...
class InstallDocker
{
    ...
    public function handle(Server $server)
    {
        ...
        if (! $supported_os_type) {
            throw new \Exception('Server OS type is not supported for automated installation. Please install Docker manually before continuing: <a target="_blank" class="underline" href="#/installation#manually">documentation</a>.');
        }
        ...
        if (! $server->sslCertificates()->where('is_ca_certificate', true)->exists()) {
            ...
            $commands = collect([
                ...
                "echo '{$serverCert->ssl_certificate}' > $caCertPath/saturn-ca.crt",
        ...
        if (isDev() && $server->id === 0) {
            ...
            $command = $command->merge([
                ...
                "echo '{$config}' | base64 -d | tee /etc/docker/daemon.json.saturn > /dev/null",
                'jq . /etc/docker/daemon.json.saturn | tee /etc/docker/daemon.json.saturn.pretty > /dev/null',
                'mv /etc/docker/daemon.json.saturn.pretty /etc/docker/daemon.json.saturn',
                "jq -s '.[0] * .[1]' /etc/docker/daemon.json.saturn /etc/docker/daemon.json | tee /etc/docker/daemon.json.appended > /dev/null",
            ...
            if ($server->isSwarm()) {
                $command = $command->merge([
                    'docker network create --attachable --driver overlay saturn-overlay >/dev/null 2>&1 || true',
                ]);
            } else {
                $command = $command->merge([
                    'docker network create --attachable saturn >/dev/null 2>&1 || true',
                ]);
                $command = $command->merge([
                    "echo 'Done!'",
                ]);
