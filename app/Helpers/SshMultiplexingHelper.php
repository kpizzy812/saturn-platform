...
class SshMultiplexingHelper
{
    ...
    public static function ensureMultiplexedConnection(Server $server): bool
    {
        ...
        $checkCommand .= escapeshellarg($server->user).'@'.escapeshellarg($server->ip);
    ...
    public static function establishNewMultiplexedConnection(Server $server): bool
    {
        ...
        $establishCommand .= escapeshellarg($server->user).'@'.escapeshellarg($server->ip);
    ...
    public static function removeMuxFile(Server $server)
    {
        ...
        $closeCommand .= escapeshellarg($server->user).'@'.escapeshellarg($server->ip);
    ...
    public static function generateScpCommand(Server $server, string $source, string $dest)
    {
        ...
        if ($server->isIpv6()) {
            $scp_command .= escapeshellarg($source).' '.escapeshellarg($server->user).'@['.escapeshellarg($server->ip).']:'.escapeshellarg($dest);
        } else {
            $scp_command .= escapeshellarg($source).' '.escapeshellarg($server->user).'@'.escapeshellarg($server->ip).':'.escapeshellarg($dest);
        }
    ...
    public static function generateSshCommand(Server $server, string $command, bool $disableMultiplexing = false)
    {
        ...
        $ssh_command .= escapeshellarg($server->user).'@'.escapeshellarg($server->ip)." 'bash -se' << \\$delimiter".PHP_EOL
            .$command.PHP_EOL
            .$delimiter;
    ...
    {
        return config('constants.ssh.mux_enabled') && ! config('constants.saturn.is_windows_docker_desktop');
    }
    ...
    public static function isConnectionHealthy(Server $server): bool
    {
        ...
        $healthCommand .= escapeshellarg($server->user).'@'.escapeshellarg($server->ip)." 'echo \"health_check_ok\"'";
