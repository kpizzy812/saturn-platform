...
class SyncBunny extends Command
{
    ...
    private function syncReleasesToGitHubRepo(): bool
    {
        ...
        try {
            ...
            $timestamp = time();
            $tmpDir = sys_get_temp_dir().'/saturn-cdn-'.$timestamp;
            $branchName = 'update-releases-'.$timestamp;

            // Clone the repository
            $this->info('Cloning saturn-cdn repository...');
            $output = [];
            exec('gh repo clone saturnplatform/saturn-cdn '.escapeshellarg($tmpDir).' 2>&1', $output, $returnCode);
            if ($returnCode !== 0) {
            ...
            $prCommand = 'gh pr create --repo saturnplatform/saturn-cdn --title '.escapeshellarg($prTitle).' --body '.escapeshellarg($prBody).' --base main --head '.escapeshellarg($branchName).' 2>&1';
            $output = [];
    ...
    private function syncVersionsToGitHubRepo(string $versionsLocation, bool $nightly = false): bool
    {
        ...
        try {
            ...
            $actualVersion = data_get($json, 'saturn.v4.version');

            $timestamp = time();
            $tmpDir = sys_get_temp_dir().'/saturn-cdn-versions-'.$timestamp;
    ...
    public function handle()
    {
        ...
        try {
            ...
            if ($only_template) {
                ...
                $json = json_decode($file, true);
                $actual_version = data_get($json, 'saturn.v4.version');
