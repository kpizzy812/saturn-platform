...
class Init extends Command
{
    ...
    public function handle()
    {
        ...
        $this->restoreSaturnDbBackup();
        $this->updateUserEmails();
        //
        $this->updateTraefikLabels();
        $this->cleanupUnusedNetworkFromSaturnProxy();
        ...
        try {
            ...
            echo "Continuing with initialization - cleanup errors will not prevent Saturn Platform from starting\n";
        }
        try {
        ...
        if (! is_null(config('constants.saturn.autoupdate', null))) {
            if (config('constants.saturn.autoupdate') == true) {
    ...
    private function cleanupUnusedNetworkFromSaturnProxy()
    {
        foreach ($this->servers as $server) {
            ...
            try {
                ...
                foreach ($removeNetworks as $network) {
                    ...
                    if (empty($out)) {
                        $commands->push("docker network disconnect $network saturn-proxy >/dev/null 2>&1 || true");
                        $commands->push("docker network rm $network >/dev/null 2>&1 || true");
                    } else {
                        $data = collect(json_decode($out, true));
                        if ($data->count() === 1) {
                            // If only saturn-proxy itself is connected to that network (it should not be possible, but who knows)
                            $isSaturnProxyItself = data_get($data->first(), 'Name') === 'saturn-proxy';
                            if ($isSaturnProxyItself) {
                                $commands->push("docker network disconnect $network saturn-proxy >/dev/null 2>&1 || true");
                                $commands->push("docker network rm $network >/dev/null 2>&1 || true");
                            }
                        }
                ...
                echo "Error in cleaning up unused networks from saturn proxy: {$e->getMessage()}\n";
            }
        }
    }

    private function restoreSaturnDbBackup()
    {
        if (version_compare('4.0.0-beta.179', config('constants.saturn.version'), '<=')) {
            try {
                ...
                echo "Error in restoring saturn db backup: {$e->getMessage()}\n";
            }
        }
    }
    ...
    {
        if (version_compare('4.0.0-beta.298', config('constants.saturn.version'), '<=')) {
