...
class StandaloneMysql extends BaseModel
{
     ...
     */
    protected $fillable = [
        'uuid', 'name', 'description', 'mysql_user', 'mysql_password', 'mysql_root_password',
        'mysql_database', 'mysql_conf', 'image', 'status', 'is_public', 'public_port',
        'ports_mappings', 'limits_memory', 'limits_memory_swap', 'limits_memory_swappiness',
        'limits_memory_reservation', 'limits_cpus', 'limits_cpuset', 'limits_cpu_shares',
        'started_at', 'environment_id', 'destination_id', 'destination_type',
        'is_log_drain_enabled', 'config_hash', 'last_online_at',
    ...
    public function getActivitylogOptions(): LogOptions
    {
        return LogOptions::defaults()
            ->logOnly(['name', 'description', 'mysql_database', 'mysql_user', 'status'])
            ->logOnlyDirty()
            ->dontSubmitEmptyLogs();
    }
            ...
            get: function () {
                ...
                if ($this->enable_ssl) {
                    ...
                    if (in_array($this->ssl_mode, ['VERIFY_CA', 'VERIFY_IDENTITY'])) {
                        $url .= '&ssl-ca=/etc/ssl/certs/saturn-ca.crt';
                    }
                }
