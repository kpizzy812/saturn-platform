...
class Server extends BaseModel
{
    ...
    {
        $banner =
            "# This file is generated by Saturn Platform, do not edit it manually.\n".
...
$schema://$host {
    handle /app/* {
        reverse_proxy saturn-realtime:6001
    }
    handle /terminal/ws {
        reverse_proxy saturn-realtime:6002
    }
    reverse_proxy saturn:8080
}";
    ...
    public function reloadCaddy()
    {
        return instant_remote_process([
            'docker exec saturn-proxy caddy reload --config /config/caddy/Caddyfile.autosave',
        ], $this);
    }

    public function proxyPath()
    {
        $base_path = config('constants.saturn.base_config_path');
    ...
    public function getCpuMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            ...
            $cpu = instant_remote_process(["docker exec saturn-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->sentinel_token}\" http://localhost:8888/api/cpu/history?from=$from'"], $this, false);
    ...
    public function getMemoryMetrics(int $mins = 5)
    {
        if ($this->isMetricsEnabled()) {
            $from = now()->subMinutes($mins)->toIso8601ZuluString();
            $memory = instant_remote_process(["docker exec saturn-sentinel sh -c 'curl -H \"Authorization: Bearer {$this->settings->sentinel_token}\" http://localhost:8888/api/memory/history?from=$from'"], $this, false);
    ...
    public function loadUnmanagedContainers(): Collection
    {
        if ($this->isFunctional()) {
            ...
            $containers = $containers->map(function ($container) {
                ...
                if (! str($labels)->contains('saturn.managed')) {
                    return $container;
                }
    ...
    public function databases()
    {
        ...
        return $postgresqls
            ...
            ->concat($dragonflies)
            ->concat($clickhouses)
            ->filter(fn ($item) => data_get($item, 'name') !== 'saturn-db');
    }
    ...
    public function validateDockerEngine($throwError = false)
    {
        ...
        $this->settings->save();
        $this->validateSaturnNetwork(isSwarm: false, isBuildServer: $this->settings->is_build_server);
    ...
    public function validateDockerSwarm()
    {
        ...
        $this->settings->is_usable = true;
        $this->settings->save();
        $this->validateSaturnNetwork(isSwarm: true);
    ...
    public function validateSaturnNetwork($isSwarm = false, $isBuildServer = false)
    {
        if ($isBuildServer) {
            return;
        }
        if ($isSwarm) {
            return instant_remote_process(['docker network create --attachable --driver overlay saturn-overlay >/dev/null 2>&1 || true'], $this, false);
        } else {
            return instant_remote_process(['docker network create saturn --attachable >/dev/null 2>&1 || true'], $this, false);
        }
    }
        ...
        try {
            ...
            if ($caCertificate) {
                ...
                $commands = collect([
                    ...
                    "echo '{$certificateContent}' > $caCertPath/saturn-ca.crt",
