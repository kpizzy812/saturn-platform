    ...
    private function check_git_if_build_needed(): void
    {
        ...
        if ($private_key) {
            ...
            $this->execute_remote_command(
                ...
                [
                    executeInDocker($this->deployment_uuid, "GIT_SSH_COMMAND=\"ssh -o ConnectTimeout=30 -p {$this->customPort} -o Port={$this->customPort} -o LogLevel=ERROR -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null\" git ls-remote {$this->fullRepoUrl} ".escapeshellarg($lsRemoteRef)),
                    'hidden' => true,
                    'save' => 'git_commit_sha',
                ]
            );
