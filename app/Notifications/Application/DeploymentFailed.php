...
class DeploymentFailed extends CustomEmailNotification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        if ($pull_request_id === 0) {
            $mail->subject('Saturn Platform: Deployment failed of '.$this->application_name.'.');
        } else {
            $fqdn = $this->preview->fqdn;
            $mail->subject('Saturn Platform: Deployment failed of pull request #'.$this->preview->pull_request_id.' of '.$this->application_name.'.');
        }
    ...
    {
        if ($this->preview) {
            $message = 'Saturn Platform: Pull request #'.$this->preview->pull_request_id.' of '.$this->application_name.' ('.$this->preview->fqdn.') deployment failed: ';
        } else {
            $message = 'Saturn Platform: Deployment failed of '.$this->application_name.' ('.$this->fqdn.'): ';
        }
        $buttons[] = [
