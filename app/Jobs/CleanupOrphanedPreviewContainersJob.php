 ...
 *
 * It scans all functional servers for containers with the `saturn.pullRequestId` label
...
class CleanupOrphanedPreviewContainersJob implements ShouldBeEncrypted, ShouldBeUnique, ShouldQueue
{
    ...
    private function getPRContainersOnServer(Server $server): \Illuminate\Support\Collection
    {
        try {
            $output = instant_remote_process([
                "docker ps -a --filter 'label=saturn.pullRequestId' --format '{{json .}}'",
            ], $server, false);
    ...
    private function extractPullRequestId($container): ?int
    {
        ...
        if (preg_match('/saturn\.pullRequestId=(\d+)/', $labels, $matches)) {
    ...
    {
        $labels = data_get($container, 'Labels', '');
        if (preg_match('/saturn\.applicationId=(\d+)/', $labels, $matches)) {
            return (int) $matches[1];
        }
