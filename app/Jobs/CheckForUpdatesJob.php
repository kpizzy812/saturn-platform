...
class CheckForUpdatesJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $response = Http::retry(3, 1000)->get(config('constants.saturn.versions_url'));
            if ($response->successful()) {
                ...
                if ($existingSaturnVersion && version_compare($existingSaturnVersion, $bestVersion, '>')) {
                    Log::warning('CDN served older Saturn Platform version than cache', [
                        'cdn_version' => $latest_version,
                        'cached_version' => $existingSaturnVersion,
                        'current_version' => $current_version,
                    ]);
                    $bestVersion = $existingSaturnVersion;
                }
                ...
                // Only mark new version available if Saturn Platform version actually increased
