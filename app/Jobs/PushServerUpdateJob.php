...
class PushServerUpdateJob implements ShouldBeEncrypted, ShouldQueue, Silenced
{
    ...
    {
        Log::error('PushServerUpdateJob permanently failed', [
    ...
    {
        // Properties are initialized in the constructor; no need for ??= fallback
        ...
        foreach ($this->containers as $container) {
            ...
            $saturn_managed = $labels->has('saturn.managed');
            ...
            if ($name === 'saturn-log-drain' && $this->isRunning($containerStatus)) {
                $this->foundLogDrainContainer = true;
            }
            if ($labels->has('saturn.applicationId')) {
                $applicationId = $labels->get('saturn.applicationId');
                $pullRequestId = $labels->get('saturn.pullRequestId', '0');
                try {
                    if ($pullRequestId === '0') {
                ...
                $subType = $labels->get('saturn.service.subType');
                $subId = $labels->get('saturn.service.subId');
                ...
                $type = $labels->get('saturn.type');
                if ($name === 'saturn-proxy' && $this->isRunning($containerStatus)) {
