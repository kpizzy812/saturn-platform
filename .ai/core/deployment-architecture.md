# Saturn Platform Deployment Architecture

## Deployment Philosophy

Saturn Platform orchestrates **Docker-based deployments** across multiple servers with automated configuration generation, zero-downtime deployments, and comprehensive monitoring.
...
```
Git Repository → Webhook → Saturn Platform → Build & Deploy
```
...
Saturn Platform provides settings to preserve Docker build cache across deployments, addressing cache invalidation issues.

#### The Problem

By default, Saturn Platform injects `ARG` statements into user Dockerfiles for build-time variables. This breaks Docker's cache mechanism because:
...
|---------|---------|-------------|
| `inject_build_args_to_dockerfile` | `true` | Controls whether Saturn Platform adds `ARG` statements to Dockerfile |
...
```dockerfile
# Saturn Platform modifies your Dockerfile to add:
FROM node:20
ARG MY_VAR=value
ARG SATURN_URL=...
...
- Saturn Platform does NOT modify the Dockerfile
...
- `app/Jobs/ApplicationDeploymentJob.php`:
  - `set_saturn_variables()` - Conditionally adds SOURCE_COMMIT to Docker build context based on `include_source_commit_in_build` setting
  - `generate_saturn_env_variables(bool $forBuildTime)` - Distinguishes build-time vs. runtime variables; excludes cache-busting variables like SOURCE_COMMIT from build context unless explicitly enabled
...
## Saturn Platform Docker Compose Extensions

Saturn Platform extends standard Docker Compose with custom fields (often called "magic fields") that provide Saturn Platform-specific functionality. These extensions are processed during deployment and stripped before sending the final compose file to Docker, maintaining full compatibility with Docker's compose specification.

### Overview

**Why Custom Fields?**
- Enable Saturn Platform-specific features without breaking Docker Compose compatibility
...
2. Saturn Platform parses and processes custom fields (creates files, stores settings)
...
- Validation: `tests/Unit/StripSaturn PlatformCustomFieldsTest.php`
...
**Function:** `stripSaturn PlatformCustomFields()` in `bootstrap/helpers/docker.php`
...
$yaml_compose = stripSaturn PlatformCustomFields($yaml_compose);  // Strip custom fields
...
Custom fields are **Saturn Platform-specific** and won't work with standalone `docker compose` CLI:

```bash
# ❌ Won't work - Docker doesn't recognize custom fields
docker compose -f compose.yaml up

# ✅ Works - Use Saturn Platform's deployment (strips custom fields first)
# Deploy through Saturn Platform UI or API
```
...
**Unit Tests:**
- `tests/Unit/StripSaturn PlatformCustomFieldsTest.php` - Custom field stripping logic
