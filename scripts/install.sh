...
ENV_FILE="/data/saturn/source/.env"
DOCKER_VERSION="27.0"
# TODO: Ask for a user
CURRENT_USER=$USER
...
echo ""
echo "=========================================="
echo "   Saturn Platform Installation - ${DATE}"
echo "=========================================="
echo ""
echo "Welcome to Saturn Platform Installer!"
echo "This script will install everything for you. Sit back and relax."
echo "Source code: https://github.com/coollabsio/saturn/blob/v4.x/scripts/install.sh"
...
if [ -f "/data/saturn/source/.env" ] && [ "$DOCKER_POOL_BASE_PROVIDED" = false ] && [ "$DOCKER_POOL_SIZE_PROVIDED" = false ]; then
    ENV_DOCKER_ADDRESS_POOL_BASE=$(grep -E "^DOCKER_ADDRESS_POOL_BASE=" /data/saturn/source/.env | cut -d '=' -f2 || true)
    ENV_DOCKER_ADDRESS_POOL_SIZE=$(grep -E "^DOCKER_ADDRESS_POOL_SIZE=" /data/saturn/source/.env | cut -d '=' -f2 || true)
...
mkdir -p /data/saturn/{source,ssh,applications,databases,backups,services,proxy,sentinel}
mkdir -p /data/saturn/ssh/{keys,mux}
mkdir -p /data/saturn/proxy/dynamic

chown -R 9999:root /data/saturn
chmod -R 700 /data/saturn

INSTALLATION_LOG_WITH_DATE="/data/saturn/source/installation-${DATE}.log"
...
echo "| Saturn Platform           | $LATEST_VERSION"
...
if [ "$SSH_DETECTED" = "false" ]; then
    ...
    *)
        ...
        echo -e "Please make sure it is installed and running, otherwise Saturn Platform cannot connect to the host system. \n"
        echo "###############################################################################"
        exit 1
        ;;
    esac
...
if [ "$SSH_PERMIT_ROOT_LOGIN" = "yes" ] || [ "$SSH_PERMIT_ROOT_LOGIN" = "without-password" ] || [ "$SSH_PERMIT_ROOT_LOGIN" = "prohibit-password" ]; then
    ...
    echo "   If you have problems with SSH, please read this: https://saturn.io/docs/knowledge-base/server/openssh"
fi
...
if [ -x "$(command -v snap)" ]; then
    ...
    if [ "$SNAP_DOCKER_INSTALLED" = "true" ]; then
        ...
        echo "   Please note that Saturn Platform does not support Docker installed via snap."
...
# Create saturn configuration with or without address pools based on whether they were explicitly provided
...
EOL
    ...
    if [ -f /etc/docker/daemon.json ] && jq -e '.["log-driver"] == "json-file" and .["log-opts"]["max-size"] == "10m" and .["log-opts"]["max-file"] == "3"' /etc/docker/daemon.json >/dev/null 2>&1; then
        ...
        cat >/etc/docker/daemon.json.saturn <<EOL
{
  "log-driver": "json-file",
  "log-opts": {
    "max-size": "10m",
...
curl -fsSL -L $CDN/docker-compose.yml -o /data/saturn/source/docker-compose.yml &
PID1=$!
curl -fsSL -L $CDN/docker-compose.prod.yml -o /data/saturn/source/docker-compose.prod.yml &
PID2=$!
curl -fsSL -L $CDN/.env.production -o /data/saturn/source/.env.production &
PID3=$!
curl -fsSL -L $CDN/upgrade.sh -o /data/saturn/source/upgrade.sh &
PID4=$!
...
if [ -f "$ENV_FILE" ]; then
    ...
    awk -F '=' '!seen[$1]++' "$ENV_FILE" "/data/saturn/source/.env.production" > "$ENV_FILE.tmp" && mv "$ENV_FILE.tmp" "$ENV_FILE"
    ...
    cp "/data/saturn/source/.env.production" "$ENV_FILE"
fi
...
set +e
IS_SATURN_VOLUME_EXISTS=$(docker volume ls | grep saturn-db | wc -l)
set -e

if [ "$IS_SATURN_VOLUME_EXISTS" -eq 0 ]; then
    echo " - Generating SSH key."
    test -f /data/saturn/ssh/keys/id.$CURRENT_USER@host.docker.internal && rm -f /data/saturn/ssh/keys/id.$CURRENT_USER@host.docker.internal
    test -f /data/saturn/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub && rm -f /data/saturn/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
    ssh-keygen -t ed25519 -a 100 -f /data/saturn/ssh/keys/id.$CURRENT_USER@host.docker.internal -q -N "" -C saturn
    chown 9999 /data/saturn/ssh/keys/id.$CURRENT_USER@host.docker.internal
    sed -i "/saturn/d" ~/.ssh/authorized_keys
    cat /data/saturn/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub >>~/.ssh/authorized_keys
    rm -f /data/saturn/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
fi
...
log_section "Step 9/9: Installing Saturn Platform"
echo "9/9 Installing Saturn Platform ($LATEST_VERSION)..."
echo -e " - It could take a while based on your server's performance, network speed, stars, etc."
echo -e " - Please wait."
getAJoke

if [[ $- == *x* ]]; then
    bash -x /data/saturn/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}" "true"
else
    bash /data/saturn/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}" "true"
fi
echo " - Saturn Platform installed successfully."
echo " - Waiting for Saturn Platform to be ready..."

# Wait for upgrade.sh background process to complete
# upgrade.sh writes status to /data/saturn/source/.upgrade-status
# Status file format: step|message|timestamp
# Step 6 = "Upgrade complete", file deleted 10 seconds after
UPGRADE_STATUS_FILE="/data/saturn/source/.upgrade-status"
MAX_WAIT=180
WAITED=0
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if [ -f "$UPGRADE_STATUS_FILE" ]; then
        ...
        if [ "$STATUS" = "6" ]; then
            ...
            echo " - Please check the upgrade logs: /data/saturn/source/upgrade-*.log"
            exit 1
        else
...
echo " - Verifying Saturn Platform is healthy..."
HEALTH_WAIT=60
HEALTH_WAITED=0
while [ $HEALTH_WAITED -lt $HEALTH_WAIT ]; do
    HEALTH=$(docker inspect --format='{{.State.Health.Status}}' saturn 2>/dev/null || echo "unknown")
    if [ "$HEALTH" = "healthy" ]; then
        log "Saturn Platform container is healthy"
        echo " - Saturn Platform is ready!"
        break
    fi
    sleep 2
...
if [ "$HEALTH" != "healthy" ]; then
    echo " - ERROR: Saturn Platform container is not healthy after ${HEALTH_WAIT}s. Status: $HEALTH"
    echo " - Please check: docker logs saturn"
    exit 1
fi
echo -e "\033[0;35m
   ____                            _         _       _   _                 _
...
if [ -n "$IPV4_PUBLIC_IP" ]; then
    echo -e "You can access Saturn Platform through your Public IPV4: http://$IPV4_PUBLIC_IP:8000"
fi
if [ -n "$IPV6_PUBLIC_IP" ]; then
    echo -e "You can access Saturn Platform through your Public IPv6: http://[$IPV6_PUBLIC_IP]:8000"
fi
...
echo -e "\nWARNING: It is highly recommended to backup your Environment variables file (/data/saturn/source/.env) to a safe location, outside of this server (e.g. into a Password Manager).\n"

log_section "Installation Complete"
log "Saturn Platform installation completed successfully"
