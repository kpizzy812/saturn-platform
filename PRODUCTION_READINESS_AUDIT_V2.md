...
|-----------|-----|--------|------------|
| Безопасность | 25% | 88% | 22.0% |
| Обработка ошибок | 15% | 82% | 12.3% |
| API валидация | 15% | 80% | 12.0% |
| Фронтенд | 15% | 88% | 13.2% |
| База данных | 15% | 82% | 12.3% |
| Инфраструктура | 15% | 85% | 12.75% |
| **ИТОГО** | **100%** | — | **~85%** |
...
### H-V1: Чувствительные данные в API ответах — FIXED
**Файл:** 6 моделей `*NotificationSettings.php` + `routes/web/settings.php`
**Статус:** ИСПРАВЛЕНО
**Суть:** API endpoint возвращал объекты настроек уведомлений со всеми чувствительными полями (smtp_password, webhook URLs, tokens).
**Fix:** Добавлен `$hidden` массив во все 6 моделей. `makeVisible()` в Inertia-роутах чтобы фронтенд мог отображать значения для авторизованного пользователя.

### H-V2: SSH port без валидации диапазона — FIXED
**Файл:** `app/Http/Controllers/Api/ServersController.php:520, 687`
**Статус:** ИСПРАВЛЕНО
**Fix:** `'port' => 'integer|nullable|min:1|max:65535'` — добавлено в create и update
...
### H-V6: `retry_after = 86400` секунд (24 часа) — NOT A BUG
**Файл:** `config/queue.php:68`
**Статус:** ОПРОВЕРГНУТО
`retry_after` ДОЛЖНО быть больше самого долгого job timeout (ApplicationDeploymentJob: 3600с).
Значение 86400 предотвращает дублирование долгих джобов. Корректное поведение.
