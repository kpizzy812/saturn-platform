...
function getCurrentApplicationContainerStatus(Server $server, int $id, ?int $pullRequestId = null, ?bool $includePullrequests = false): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=saturn.applicationId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);

        $containers = $containers->map(function ($container) use ($pullRequestId, $includePullrequests) {
            $labels = data_get($container, 'Labels');
            $containerName = data_get($container, 'Names');
            $hasPrLabel = str($labels)->contains('saturn.pullRequestId=');
            $prLabelValue = null;

            if ($hasPrLabel) {
                preg_match('/saturn\.pullRequestId=(\d+)/', $labels, $matches);
            ...
            if ($pullRequestId !== null && $pullRequestId !== 0 && str($labels)->contains("saturn.pullRequestId={$pullRequestId}")) {
                return $container;
            }
...
function getCurrentServiceContainerStatus(Server $server, int $id): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=saturn.serviceId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);
...
function defaultDatabaseLabels($database)
{
    ...
    $labels->push('saturn.type=database');
    $labels->push('saturn.databaseId='.$database->id);
    $labels->push('saturn.resourceName='.Str::slug($database->name));
    $labels->push('saturn.serviceName='.Str::slug($database->name));
    $labels->push('saturn.projectName='.Str::slug($database->project()->name));
    $labels->push('saturn.environmentName='.Str::slug($database->environment->name));
    $labels->push('saturn.database.subType='.$database->type());
...
function defaultLabels($id, $name, string $projectName, string $resourceName, string $environment, $pull_request_id = 0, string $type = 'application', $subType = null, $subId = null, $subName = null)
{
    $labels = collect([]);
    $labels->push('saturn.managed=true');
    $labels->push('saturn.version='.config('constants.saturn.version'));
    $labels->push('saturn.'.$type.'Id='.$id);
    $labels->push("saturn.type=$type");
    $labels->push('saturn.name='.$name);
    $labels->push('saturn.resourceName='.Str::slug($resourceName));
    $labels->push('saturn.projectName='.Str::slug($projectName));
    $labels->push('saturn.serviceName='.Str::slug($subName ?? $resourceName));
    $labels->push('saturn.environmentName='.Str::slug($environment));

    $labels->push('saturn.pullRequestId='.$pull_request_id);
    if ($type === 'service') {
        $subId && $labels->push('saturn.service.subId='.$subId);
        $subType && $labels->push('saturn.service.subType='.$subType);
        $subName && $labels->push('saturn.service.subName='.Str::slug($subName));
    }
...
function fqdnLabelsForTraefik(string $uuid, Collection $domains, bool $is_force_https_enabled = false, $onlyPort = null, ?Collection $serviceLabels = null, ?bool $is_gzip_enabled = true, ?bool $is_stripprefix_enabled = true, ?string $service_name = null, bool $generate_unique_uuid = false, ?string $image = null, string $redirect_direction = 'both', bool $is_http_basic_auth_enabled = false, ?string $http_basic_auth_username = null, ?string $http_basic_auth_password = null)
{
    ...
    if ($serviceLabels) {
        $middlewares_from_labels = $serviceLabels->map(function ($item) {
            ...
            if (preg_match('/saturn\.traefik\.middlewares=(.*)/', $item, $matches)) {
...
/**
 * Remove Saturn Platform's custom Docker Compose fields from parsed YAML array
 *
 * Saturn Platform extends Docker Compose with custom fields that are processed during
 ...
 */
function stripSaturnCustomFields(array $yamlCompose): array
{
...
function validateComposeFile(string $compose, int $server_id): string|Throwable
{
    ...
    try {
        ...
        // Remove Saturn Platform's custom fields before Docker validation
        $yaml_compose = stripSaturnCustomFields($yaml_compose);
