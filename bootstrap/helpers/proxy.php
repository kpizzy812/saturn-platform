...
function collectProxyDockerNetworksByServer(Server $server)
{
    ...
    $networks = instant_remote_process(['docker inspect --format="{{json .NetworkSettings.Networks }}" saturn-proxy'], $server, false);
...
function collectDockerNetworksByServer(Server $server)
{
    ...
    if ($server->isSwarm()) {
        if ($networks->count() === 0) {
            $networks = collect(['saturn-overlay']);
            $allNetworks = collect(['saturn-overlay']);
        }
    } else {
        if ($networks->count() === 0) {
            $networks = collect(['saturn']);
            $allNetworks = collect(['saturn']);
        }
    }
...
function connectProxyToNetworks(Server $server)
{
    ...
    // Additionally connect to networks of all Traefik-enabled containers discovered via Docker.
...
function extractCustomProxyCommands(Server $server, string $existing_config): array
{
    ...
    try {
        ...
        // Define default commands that Saturn Platform generates
...
function generateDefaultProxyConfiguration(Server $server, array $custom_commands = [])
{
    ...
    if ($proxy_type === ProxyTypes::TRAEFIK->value) {
        ...
        if (isDev()) {
            ...
            $config['services']['traefik']['volumes'][] = '/var/lib/docker/volumes/saturn_dev_saturn_data/_data/proxy/:/traefik';
        } else {
...
function getExactTraefikVersionFromContainer(Server $server): ?string
{
    try {
        ...
        $versionCommand = "docker exec saturn-proxy traefik version 2>/dev/null | grep -oP 'Version:\s+\K\d+\.\d+\.\d+'";
        ...
        $labelCommand = "docker inspect saturn-proxy --format '{{index .Config.Labels \"org.opencontainers.image.version\"}}' 2>/dev/null";
