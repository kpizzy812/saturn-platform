...
function applicationParser(Application $resource, int $pull_request_id = 0, ?int $preview_id = null, ?string $commit = null): Collection
{
    ...
    foreach ($services as $serviceName => $service) {
        ...
        // Add Saturn Platform defined environments
        ...
        if ($magicEnvironments->count() > 0) {
            // Generate Saturn Platform environment variables
    ...
    foreach ($services as $serviceName => $service) {
            ...
            foreach ($volumes as $index => $volume) {
                ...
                if ($type->value() === 'bind') {
                    if ($source->value() === '/var/run/docker.sock') {
                        ...
                        if (isDev()) {
                            if ((int) $resource->compose_parsing_version >= 4) {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/saturn_dev_saturn_data/_data/applications/'.$uuid);
                            } else {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/saturn_dev_saturn_data/_data/applications/'.$uuid);
                            }
                        }
            ...
            if ($key->value() === $parsedValue->value()) {
                ...
                if ($value->startsWith('$')) {
                    ...
                    if ($originalValue->value() === $value->value()) {
                        // This means the variable does not have a default value, so it needs to be created in Saturn Platform
        ...
        if ($originalResource->environment_variables->where('key', 'SATURN_BRANCH')->isEmpty()) {
            $saturnEnvironments->put('SATURN_BRANCH', "\"{$branch}\"");
        }

        // Add SATURN_RESOURCE_UUID to environment
        if ($resource->environment_variables->where('key', 'SATURN_RESOURCE_UUID')->isEmpty()) {
            $saturnEnvironments->put('SATURN_RESOURCE_UUID', "{$resource->uuid}");
        }

        // Add SATURN_CONTAINER_NAME to environment
        if ($resource->environment_variables->where('key', 'SATURN_CONTAINER_NAME')->isEmpty()) {
        ...
        if ($resource->build_pack === 'dockercompose') {
            foreach ($domains as $forServiceName => $domain) {
                ...
                if (filled($parsedDomain)) {
                    ...
                    $saturnEnvironments->put('SERVICE_URL_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'), $saturnUrl->__toString());
                    $saturnEnvironments->put('SERVICE_FQDN_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'), $saturnFqdn);
                    $resource->environment_variables()->updateOrCreate([
        ...
        $isDatabase = isDatabaseImage($image, $service);
        // Add SATURN_FQDN & SATURN_URL to environment
        ...
        add_saturn_default_environment_variables($resource, $saturnEnvironments, $resource->environment_variables);
        ...
        // Auto-inject .env file so Saturn Platform environment variables are available inside containers
    ...
    // Parse the original compose again to create a clean version without Saturn Platform additions
    try {
        ...
        foreach ($environment as $key => $value) {
            ...
            if ($key->startsWith('SERVICE_FQDN_') || $key->startsWith('SERVICE_URL_')) {
                ...
                if ($isServiceApplication && is_null($savedService->fqdn)) {
                    ...
                    if ((int) $resource->compose_parsing_version >= 5 && version_compare(config('constants.saturn.version'), '4.0.0-beta.420.7', '>=')) {
            ...
            foreach ($volumes as $index => $volume) {
                ...
                if ($type->value() === 'bind') {
                    if ($source->value() === '/var/run/docker.sock') {
                        ...
                        if (isDev()) {
                            if ((int) $resource->compose_parsing_version >= 4) {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/saturn_dev_saturn_data/_data/services/'.$uuid);
                            } else {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/saturn_dev_saturn_data/_data/applications/'.$uuid);
                            }
                        }
