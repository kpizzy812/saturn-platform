...
<div class="flex flex-col items-center justify-center min-h-screen">
    <div>
        <p class="font-mono font-semibold text-7xl text-warning">400</p>
        <h1 class="mt-4 font-bold tracking-tight text-white">Bad Request</h1>
        @if ($exception->getMessage())
            <p class="text-base leading-7 text-red-500">{{ $exception->getMessage() }}</p>
        @else
            <p class="text-base leading-7 text-neutral-400">The request could not be understood by the server due to malformed syntax.</p>
        @endif
